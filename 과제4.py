...
for i, e in enumerate(img_list1,1):
    with open(savepath+"평점순"+str(i)+".txt","wt") as f:
        f.write(e.select_one("h4.block_title > a").string)
    fullfilename = os.path.join(savepath, savepath+'평점순'+str(i)+'.png')
    req.urlretrieve(e.select_one("div.block_media > a > img")['src'],fullfilename)
for i, e in enumerate(img_list2,1):
    with open(savepath+"학생수순"+str(i)+".txt","wt") as f:
        f.write(e.select_one("h4.block_title > a").string)
    fullfilename = os.path.join(savepath, savepath+'학생수순'+str(i)+'.png')
    req.urlretrieve(e.select_one("div.block_media > a > img")['src'],fullfilename)
